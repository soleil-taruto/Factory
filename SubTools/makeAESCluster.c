...
static void MEDF_MakeEncDataFile(autoBlock_t *fileData, autoBlock_t *rawKey, uint resId, uint tweek)
{
	...
	// encrypt-rcbc
	{
		AES128_KeyTable_t *key = AES128_CreateKeyTable(rawKey, 1);
		...
		AES128_Encrypt(key, dataBuff, dataBuff);

		for (index = 16; index < dataSize; index += 16)
		{
			XorBlock(dataBuff + index, dataBuff + index - 16);
			AES128_Encrypt(key, dataBuff + index, dataBuff + index);
		}
		AES128_ReleaseKeyTable(key);
	}
}
